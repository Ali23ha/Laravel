    ...
    <header style="background-color: #2e3e4e !important;" class="header-bar mb-3" >
        <div class="container d-flex flex-column flex-md-row align-items-center p-3">
            ...
            <div class="flex-row my-3 my-md-0">
                ...
                <a class="btn btn-sm btn-success mr-2" href="{{route('Reports-index-admin')}}">
                    Post Panel
                </a>
    ...
    @foreach($Posts as $Post)
     ...
     <div class="container py-md-5 container--narrow" style="padding-bottom: 0rem !important;">
            <div class="d-flex justify-content-between">
                    <a href="{{route('Show-post-admin',$Post->id)}}"><h2> {{$Post->title}} </h2></a>
               <span class="pt-2">
               <form class="delete-post-form d-inline" action="{{route('Delete-post-admin',$Post->id)}}" method="POST">
                  @method('DELETE')
                   @csrf
                    <button class="delete-post-button text-danger" data-toggle="tooltip" data-placement="top" title="Delete"><i class="fas fa-trash"></i>
                    </button>
               </form>
               </span>
            </div>
         ...
         <p class="text-muted font-small mb-6" >
             ...
             <div class="flex items-center space-x-4 mt-2">
                 <form action="{{ route('Posts-upvote-admin', $Post->id) }}" method="POST" >
                     @csrf
            ...
            <div class="body-content">
             ...
             <div class="flex items-center space-x-4 mt-2">
                 <form action="{{ route('Posts-downvote-admin', $Post->id) }}" method="POST" >
                     @csrf
                     <button type="submit" class="btn btn-sm position-relative rounded" style="border-radius: 6.25rem !important;color: #ffffff;
                                            background-color: #0069d9;border-color: #0069d9;">
                                            <span class="position-absolute top-0 start-100 translate-middle badge rounded-pill bg-danger">
            ...
            <div class="mt-4">
                ...
                @foreach($Post->comments as $comment)
                    <form action="{{ route('Comments-delete-admin', $comment->id) }}" method="POST" class="mt-2" style="margin: 13px -9px -32px 19px !important">
                            @csrf
                            @method('DELETE')
                    ...
                    <div class="flex items-center space-x-4 mt-2">
                        <form action="{{ route('Comments-upvote-admin', $comment->id) }}" method="POST">
                            @csrf
                            <button type="submit" class="btn btn-sm position-relative rounded" style="border-radius: 6.25rem !important;color: #ffffff;
                                        background-color: #0069d9;border-color: #0069d9;">
                                        <span class="position-absolute top-0 start-100 translate-middle badge rounded-pill bg-danger">
                    ...
                    <div class="flex items-center space-x-4 mt-2">
                        <form action="{{ route('Comments-downvote-admin', $comment->id) }}" method="POST">
                            @csrf
                            <button type="submit" class="btn btn-sm position-relative rounded" style="border-radius: 6.25rem !important;color: #ffffff;
                                        background-color: #0069d9;border-color: #0069d9;">
                                <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" fill="currentColor" class="bi bi-arrow-down" viewBox="0 0 16 16">
