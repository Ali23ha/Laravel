...
@section('content')
    <header style="background-color: #2e3e4e !important;" class="header-bar mb-3" >
        <div class="container d-flex flex-column flex-md-row align-items-center p-3">
            <h4 class="my-0 mr-md-auto font-weight-normal"><a href="{{route('Index-post-coach')}}" class="text-white">Test</a></h4>
    ...
    @foreach($Posts as $Post)
        ...
        <div class="container py-md-5 container--narrow" style="padding-bottom: 0rem !important;">
            <div class="d-flex justify-content-between">
                <a href="{{route('Show-post-coach',$Post->id)}}"><h2> {{$Post->title}} </h2></a>
                <span class="pt-2">
                    </span>
                @if(auth()->id() === $Post->user_id)
                    <span class="pt-2">
                         <a href="{{route('Edit-post-coach',$Post->id)}}" class="text-primary mr-2" data-toggle="tooltip" data-placement="top" title="Edit"><i class="fas fa-edit"></i></a>
                          <form class="delete-post-form d-inline" action="{{route('Delete-post-coach',$Post->id)}}" method="POST">
                           @method('DELETE')
                           @csrf
            ...
            <div class="flex items-center space-x-4 mt-2">
                <form action="{{ route('Posts-upvote-coach', $Post->id) }}" method="POST" >
                    @csrf
            ...
            <div class="flex items-center space-x-4 mt-2">
                <form action="{{ route('Posts-downvote-coach', $Post->id) }}" method="POST" >
                    @csrf
                    <button type="submit" class="btn btn-sm position-relative rounded" style="border-radius: 6.25rem !important;color: #ffffff;
                                        background-color: #0069d9;border-color: #0069d9;">
                                        <span class="position-absolute top-0 start-100 translate-middle badge rounded-pill bg-danger">
            ...
            <div class="mt-4">
                ...
                <button type="button" class="btn btn-danger position-relative">
                    <a style="color: white" href="{{route('Reports-create-coach',$Post->id)}}">Report </a>
                </button>
                @foreach($Post->comments as $comment)
                    @if(auth()->id() === $comment->user_id)
                        <form action="{{ route('Comments-delete-coach', $comment->id) }}" method="POST" class="mt-2" style="margin: 13px -9px -32px 19px !important">
                            @csrf
                            @method('DELETE')
                            <button style="margin-left: 665px" class="delete-post-button text-danger" data-toggle="tooltip" data-placement="top" title="Delete"><i class="fas fa-trash" style="margin-bottom: -70px;"></i>
                            </button>
                        </form>
                    @endif
                    ...
                    <div class="flex items-center space-x-4 mt-2">
                        <form action="{{ route('Comments-upvote-coach', $comment->id) }}" method="POST">
                            @csrf
                            <button type="submit" class="btn btn-sm position-relative rounded" style="border-radius: 6.25rem !important;color: #ffffff;
                                        background-color: #0069d9;border-color: #0069d9;">
                                        <span class="position-absolute top-0 start-100 translate-middle badge rounded-pill bg-danger">
                    ...
                    <div class="flex items-center space-x-4 mt-2">
                        <form action="{{ route('Comments-downvote-coach', $comment->id) }}" method="POST">
                            @csrf
                            <button type="submit" class="btn btn-sm position-relative rounded" style="border-radius: 6.25rem !important;color: #ffffff;
                                        background-color: #0069d9;border-color: #0069d9;">
                                <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" fill="currentColor" class="bi bi-arrow-down" viewBox="0 0 16 16">
