...
@section('content')
    ...
    <div class="container py-md-5 container--narrow">
        <form action="{{route('Update-post-trainee',$Post->id)}}" method="POST">
            @csrf
            @method('PUT')
            <div class="form-group">
                <label for="title" class="text-muted mb-1"><small>Title</small></label>
                <input required name="title" value="{{ old('title') }}" class="form-control form-control-lg form-control-title" type="text" placeholder="" autocomplete="off" />
            </div>
